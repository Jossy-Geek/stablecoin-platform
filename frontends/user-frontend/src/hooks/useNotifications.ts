...
import { useTransactionSocket } from './useTransactionSocket';
...
export function useNotifications(autoRefresh: boolean = true, refreshInterval: number = 30000): UseNotificationsReturn {
  // Call useUser hook unconditionally (must be before any conditional logic)
  const { user } = useUser();
  ...
  // Store userId in localStorage when available (client-side only)
  useEffect(() => {
    if (isMounted && typeof window !== 'undefined' && user?.id) {
  ...
  }, [fetchNotifications, fetchUnreadCount, isMounted]);

  // Auto-refresh unread count (client-side only)
  useEffect(() => {
